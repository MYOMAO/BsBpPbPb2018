...
void ReAnaEffMCData(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int drop, int Weight){
	...
	if(Weight == 0) finMap = new TFile(Form("MCDataMap/NoWeight/NoWeight_%d_%d.root",CentMin,CentMax));
	if(Weight == 3) finMap = new TFile(Form("MCDataMap/sPLOT/sPLOT_%d_%d.root",CentMin,CentMax));
	if(Weight > 3) finMap = new TFile(Form("MCDataMap/sPLOT/%s_%d_%d.root",WeightName.Data(),CentMin,CentMax));
