...
void ManualCutBPEM(int CentMin, int CentMax, int IsMC, int Shape, int TwoShot){
...
//	if(CentMin == 40 && CentMax == 90) func = new TF1("func","1/0.131733+TMath::Exp(-1.051514*(x-15.468517))+TMath::Exp(-0.204272*(x-31.002324))",5,50);
	...
	TFile * finTnP =  new TFile(Form("/export/d00/scratch/zzshi/CMSSW_7_5_8_patch3/Merge/BPlusAna/CrossSection/ROOTfiles/TNP2D_Bplus_Cent%d-%d.root",CentMin,CentMax));
...
//	TH1D * weights_BDT_pt_20_50 = (TH1D *) finSystWeight->Get("weights_BDT_pt_20_50");
	...
	TH1D * hCentEffInv =  (TH1D *) finEff->Get("EffBptByInv");
	...
	TFile * fout;
	if(IsMC == 2) fout = new TFile(Form("/export/d00/scratch/zzshi/CMSSW_7_5_8_patch3/Merge/BsBP2018PbPb/Codes/BsAndBP/L2L3MuonMatch/AnaData/BP/EffInfo_%d_%d.root",CentMin,CentMax),"RECREATE");
...
//				cout << "------------------ After Shit --------------------------" << endl;
			/*
				cout << "The Event = " << i << endl;
				cout << "pprimaryVertexFilter = " << pprimaryVertexFilter << endl;
				cout << "phfCoincFilter2Th4 = " << phfCoincFilter2Th4 << endl;
				cout << "pclusterCompatibilityFilter = " << pclusterCompatibilityFilter << endl;
				cout << " Btrk1Pt[j] = " <<  Btrk1Pt[j] << endl;
				cout << " BsvpvDistance[j]/BsvpvDisErr[j] =  " << BsvpvDistance[j]/BsvpvDisErr[j] << endl; 
				cout <<  " Bchi2cl[j] = " <<  Bchi2cl[j] << endl;
				cout << "TMath::Abs(Btrk1Eta[j]) = " << TMath::Abs(Btrk1Eta[j]) << endl;
				cout << "TMath::Abs(By[j]) = " << TMath::Abs(By[j]) << endl;
				cout << " TMath::Abs(PVz)<15 = " <<  TMath::Abs(PVz) << endl;
				cout << "TMath::Abs(Bmumumass[j]-3.096900) = " << TMath::Abs(Bmumumass[j]-3.096900) << endl;
				cout << "Bmu1SoftMuID = " << Bmu1SoftMuID[j] << endl;
				cout << "Bmu2SoftMuID = " << Bmu2SoftMuID[j] << endl;
				cout << " Bmu1isAcc[j] = " <<  Bmu1isAcc[j] << endl;
				cout << " Bmu2isAcc[j] = " <<  Bmu2isAcc[j] << endl;
				cout << " Bmu1isTriggered[j] = " <<  Bmu1isTriggered[j] << endl;
				cout << "Bmu2isTriggered[j] = " << Bmu2isTriggered[j] << endl;
				cout << "(Btrk1PixelHit[j]+Btrk1StripHit[j]) = " << (Btrk1PixelHit[j]+Btrk1StripHit[j]) << endl;
				cout << "(Btrk1Chi2ndf[j]/(Btrk1nStripLayer[j]+Btrk1nPixelLayer[j])) = " << (Btrk1Chi2ndf[j]/(Btrk1nStripLayer[j]+Btrk1nPixelLayer[j])) << endl;
				cout << "TMath::Abs(Btrk1PtErr[j]/Btrk1Pt[j]) = " << TMath::Abs(Btrk1PtErr[j]/Btrk1Pt[j]) << endl;
				cout << "BDT_pt_5_7[j] =  " << BDT_pt_5_7[j] << endl;
				cout << "BDT_pt_7_10[j] =  " << BDT_pt_7_10[j] << endl;
				cout << "BDT_pt_10_15[j] =  " << BDT_pt_10_15[j] << endl;
				cout << "BDT_pt_15_20[j] =  " << BDT_pt_15_20[j] << endl;
				cout << "BDT_pt_20_30[j] =  " << BDT_pt_20_30[j] << endl;
				cout << "BDT_pt_30_40[j] =  " << BDT_pt_30_40[j] << endl;
				cout << "BDT_pt_40_50[j] =  " << BDT_pt_40_50[j] << endl;
				cout << "The Event = " << i << endl;

				cout << "CentMin = " << CentMin << " hiBin =  " << hiBin <<  "   CentMax =  " << CentMax << endl;

				*/
	//			cout << "Bmass =" << Bmass[j] << endl;
