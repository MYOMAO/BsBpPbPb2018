...
//			lat->SetTextSize(ltxSetTextSize1 * 0.64);
			...
			lat->SetTextFont(42);
			lat->SetTextSize(ltxSetTextSize2);
			lat->DrawLatex(legRatio_xLowStart-0.08,legRatio_y-0.15,Form("Global uncert.: #pm %.1f %%",glbSystDown));
			...
			if(drawRef)
			{
				TLegend *legRatioRef = new TLegend(legRatioRef_xLowStart-0.06,legRatioRef_y-0.04,legRatioRef_xLowEnd-0.03,legRatio_y+0.12,NULL,"brNDC");
