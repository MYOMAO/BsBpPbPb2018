...
void BP1Bin(){
	...
	TString BsName = "BsBP1Bin/CrossSectionPbPbBPlus.root";
...
//	NPartPlot->Draw("epx0");
	...
	double NMBSystUp[_nBins ] = {0.0118};
	double NMBSystDown[_nBins ] = {0.0118};
	...
	for(int i = 0; i < _nBins; i++){
		...
		SystErrUp[i] =  sqrt(TrackingErrBP[i] * TrackingErrBP[i] + SelErrBP[i] *SelErrBP[i] + PTErrBP[i] * PTErrBP[i]  + PDFBErrBP[i]* PDFBErrBP[i] + PDFSErrBP[i]* PDFSErrBP[i] + TnPErrBPUp[i]* TnPErrBPUp[i] + MCStatBP[i] * MCStatBP[i] + SystTAA[i] * SystTAA[i] + NMBSystUp[i] * NMBSystUp[i] );
	
		SystErrDown[i] =  sqrt(TrackingErrBP[i] * TrackingErrBP[i] + SelErrBP[i] *SelErrBP[i] + PTErrBP[i] * PTErrBP[i]  + PDFBErrBP[i]* PDFBErrBP[i] + PDFSErrBP[i]* PDFSErrBP[i] + TnPErrBPDown[i]* TnPErrBPDown[i]+ MCStatBP[i] * MCStatBP[i]+ SystTAA[i] * SystTAA[i] + NMBSystDown[i] * NMBSystDown[i] );
...
//	double NMB = 11.1;
	...
	ofstream outratio("OutText/corryield_cent_0_90_Bp_New.txt");
	outratio << "NPart" << "   " <<  "xsec" << "   " << "statUncertUp" << "   " << "statUncertDown"  << "   " << "systUncert Up" << "   " << "systUncert Down" << "   " <<    "glbUncert Up"<< "   " <<  "glbUncert Down "  << endl;

	for(int i = 0; i < _nBins; i++){
		outratio <<  X[i]  << "   " << MyY[i] << "   " << StatUpBP[i] << "   " << StatDownBP[i] << "   " << SystErrUp[i]  << "   " << SystErrDown[i] << "   "   << 0.028 << "   " << 0.028 << endl;
	...
	TLatex* texcent = new TLatex(0.23,0.75,"10 GeV/c < B p_{T} < 50 GeV/c and B |y| < 2.4");
	texcent->SetNDC();
...
*/
	...
	c->SaveAs("FinalPlots/BPlusCorrYieldCent_0_90.pdf");
