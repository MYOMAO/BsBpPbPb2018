...
//	NPartPlot->Draw("epx0");
	...
	TH2F* hemptySigma=new TH2F("hemptySigma","",50,0.,300,10.,3000,15000);  
...
//	double TnPErrBs[_nBins] ={0.0531,0.0563};
	...
	double PDFSErrBP[_nBins] ={0.0139,0.0619};
//	double TnPErrBP[_nBins] ={0.06945,0.0634};


	double 	glbUncertUp[_nBins]  = {0.076,0.076};
	double	glbUncertDown[_nBins]  = {0.076,0.076};
	...
	double NMBSystDown[_nBins] ={0.0051,0.0051};
	double	NMBSystUp[_nBins ] = {0.0103,0.0103};
	for(int i = 0; i < _nBins; i++){

		SystErrUp[i] =  sqrt( TrackingErrBs[i] * TrackingErrBs[i] + SelErrBs[i] *SelErrBs[i] + PTErrBs[i] * PTErrBs[i] + AccErrBs[i]* AccErrBs[i] + PDFBErrBs[i]* PDFBErrBs[i] + PDFSErrBs[i]* PDFSErrBs[i] + TnPErrBsUp[i]* TnPErrBsUp[i] + MCStatBs[i] * MCStatBs[i]  + SystTAA[i]*SystTAA[i] + NMBSystUp[i] * NMBSystUp[i]);


		SystErrDown[i] = sqrt(TrackingErrBs[i] * TrackingErrBs[i] + SelErrBs[i] *SelErrBs[i] + PTErrBs[i] * PTErrBs[i] + AccErrBs[i]* AccErrBs[i] + PDFBErrBs[i]* PDFBErrBs[i] + PDFSErrBs[i]* PDFSErrBs[i] + TnPErrBsDown[i]* TnPErrBsDown[i] + MCStatBs[i] * MCStatBs[i]  + SystTAA[i]*SystTAA[i] + NMBSystDown[i] * NMBSystDown[i] );
...
//	double SystErr[_nBins] ={0.53,0.578};
	...
	ofstream outratio("OutText/corryield_cent_0_30_90_Bs_New.txt");
		outratio << "NPart" << "   " <<  "xsec" << "   " << "statUncertUp" << "   " << "statUncertDown" << "   " << "systUncert Up" << "   " << "systUncert Down" << "   " <<    "glbUncert Up"<< "   " <<  "glbUncert Down "  << endl;

	for(int i = 0; i < _nBins; i++){
		outratio << X[i]  << "   " << MyY[i] << "   "  << StatUpBs[i] << "   " <<  StatDownBs[i] << "   " << SystErrUp[i] << "   " << SystErrDown[i]  << "   " <<  glbUncertUp[i] << "   " <<  glbUncertDown[i]   << endl;
	}
	...
	c->SaveAs("FinalPlots/BstoBPlusCorrYield_0_30_90.pdf");
