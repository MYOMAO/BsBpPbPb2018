...
void Bs4Bins(){
	...
	gStyle->SetOptStat(0);
	TString BsName = "BsBP4Bins/CrossSectionPbPbBs.root";
...
//	double SelErrBs[nBins] ={0.378,0.0479,0.0441,0.1044};
	...
	double MCStat[nBins] ={0.288,0.0626,0.0317,0.0349};   // 2D Map MC Stat
...
//	double EffCorrDown[nBins] ={0.380,0.190,0.0570,0.0390};  //Estimated from the 33% deviation from the mean//
	...
	for(int i = 0; i < nBins; i++){
		...
		SystErrUp[i] =  sqrt(TrackingErrBs[i] * TrackingErrBs[i] + SelErrBs[i] *SelErrBs[i] + PTErrBs[i] * PTErrBs[i] + AccErrBs[i]* AccErrBs[i] + PDFBErrBs[i]* PDFBErrBs[i] + PDFSErrBs[i]* PDFSErrBs[i] + TnPErrBsUp[i]* TnPErrBsUp[i] + MCStat[i] * MCStat[i] + EffCorrUp[i] * EffCorrUp[i] );
		SystErrDown[i] =  sqrt(TrackingErrBs[i] * TrackingErrBs[i] + SelErrBs[i] *SelErrBs[i] + PTErrBs[i] * PTErrBs[i] + AccErrBs[i]* AccErrBs[i] + PDFBErrBs[i]* PDFBErrBs[i] + PDFSErrBs[i]* PDFSErrBs[i] + TnPErrBsDown[i]* TnPErrBsDown[i]+ MCStat[i] * MCStat[i]+ EffCorrDown[i] * EffCorrDown[i] );
	...
	ofstream outratio("OutText/corryield_pt_Bs_New.txt");
	outratio << "ptmin" << "   " << "ptmax" << "   " <<  "xsec" << "   " << "statUncertUp" << "   "  << "statUncertDown" << "   " << "SystUncert Up" <<  "   " << "SystUncert Down"  << "   "  <<  "glbUncert Up " << "   " <<  "glbUncert Down" << endl;

	for(int i = 0; i < nBins; i++){
		outratio << ptBinsSave[i] << "   " << ptBinsSave[i+1] << "   " << Y[i] << "   " << StatUpBs[i] << "   " << StatDownBs[i] << "   " << SystErrUp[i]  << "   " << SystErrDown[i] << "   "  <<  0.0800 << "   " << 0.0800 << endl;
	...
	TLatex *texY = new TLatex(0.17,0.63," 10 GeV/c < B p_{T} < 50 GeV/c and B |y| < 2.4");
	texY->SetNDC();
	texY->SetTextFont(42);
	...
	TLatex *texY2 = new TLatex(0.17,0.56," 7 GeV/c < B p_{T} < 10 GeV/c and 1.5 < B |y| < 2.4");
	texY2->SetNDC();
	...
	c->SetLogy();
	c->SaveAs("FinalPlots/BsCorrYieldPt.pdf");
