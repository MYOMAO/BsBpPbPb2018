...
void ReAnaEffLoop(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int NBptBins, int drop, int Weight ){
	...
	TH1D * CorrYieldHisDis = new TH1D("CorrYieldHisDis","CorrYieldHisDis",100,-3,3);

	CorrYieldHisDis->GetXaxis()->SetTitle("Corr Yield Pull");
	CorrYieldHisDis->GetYaxis()->SetTitle("Counts");
	CorrYieldHisDis->GetYaxis()->SetTitleOffset(1.5);
	CorrYieldHisDis->GetXaxis()->CenterTitle();
	CorrYieldHisDis->GetYaxis()->CenterTitle();

	double CorrYieldNom = 1.21451e+09/153050.0*164;
	double CorrYieldPull;

	const int NFiles = 900;

	for(int q = 0; q < NFiles; q++){ 

		cout << "Now Working on File =  " << q << endl;

		FileName = Form("CheckSystNuno/%s/MCSPlit/%d-%d/MC_%d.root",WeightName.Data(),CentMin,CentMax,q);
...
*/
	...
	//			cout << "i = " << i << "  CorrYieldDiff =  " << CorrYieldDiff[i] << "  Fractional Error = " << CorrYieldDiffErr[i]/CorrYieldDiff[i] << endl;
		...
		//CorrYieldPull = (FinalCorrYield *TAA - CorrYieldNom)/FinalCorrYieldErr;

		CorrYieldPull = (FinalCorrYield *TAA - CorrYieldNom)/(CorrYieldNom);

		CorrYieldHisDis->Fill(CorrYieldPull);
		fin->Close();
	}
	...
	cPull->cd();
	CorrYieldHisDis->Draw();
	cPull->SaveAs("foutCorrPull.png");

	TFile * foutCorrPull = new TFile("foutCorrPull.root","RECREATE");
	foutCorrPull->cd();
	CorrYieldHisDis->Write();
	foutCorrPull->Close();
