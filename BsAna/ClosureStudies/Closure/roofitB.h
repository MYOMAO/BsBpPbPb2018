	...
	*/
	...
	for(int i = 0; i < params_size; ++i){
		...
		if(tree=="ntKp"){
			...
			c_pull->SaveAs(Form("new/pull_signal_full%d_%s_%d_%d_Bs.png",full,variable.Data(),_count,i));
			//c_pull->SaveAs("./mcstudy/pulls_poisson_Bs.pdf");
			c_params->SaveAs(Form("new/param_signal_full%d_%s_%d_%d_Bs.png",full,variable.Data(),_count,i));
			c_errors->SaveAs(Form("new/error_signal_full%d_%s_%d_%d_Bs.png",full,variable.Data(),_count,i));
