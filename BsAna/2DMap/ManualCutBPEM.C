...
void ManualCutBPEM(int CentMin, int CentMax){
	...
	TString FileName = "../../../../BsBPInputSamples/BPData.root";
...
//	if(CentMin == 40 && CentMax == 90) func = new TF1("func","1/0.131733+TMath::Exp(-1.051514*(x-15.468517))+TMath::Exp(-0.204272*(x-31.002324))",5,50);
	...
	//TFile * finEff = new TFile(Form("EffMapFilesNew/NoWeight_%d_%d.root",CentMin,CentMax));
...
//	TH1D * hCentEffInv =  (TH1D *) finEff->Get("EffBptByInv");
	...
	TFile * fout = new TFile(Form("../Nominal/Efficiency/BeforeMerge/EffInfo_%d_%d.root",CentMin,CentMax),"RECREATE");
	fout->cd();
	...
	//			cout << "iPass = " << iPass << endl;
				...
				BEffInvErr[iPass] =  EffBptByInv->GetBinError(XLoc,YLoc);
