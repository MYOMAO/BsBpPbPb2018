...
void ManualCutPseudoData(int CentMin, int CentMax, int IsMC, int Shape, int TwoShot, int PtOpt, int pthatCut, int TEffOpt){
	...
	ntphi->SetBranchAddress("Bmu1Triggered",Bmu1Triggered);
	ntphi->SetBranchAddress("Bmu2Triggered",Bmu2Triggered);
	...
	Bool_t Bmu1TriggeredNew[NCand];
	Bool_t Bmu2TriggeredNew[NCand];
	...
	for(int i = 0; i < NEvents; i++){
		...
		for(int j =0; j < Bsize; j++){
			...
			//	if(BsizeNew > 0){
			if(NGenCand > 0 && PtOpt > -2){
				EffInfoTree->Fill();

			}
			if(BsizeNew > 0 &&  PtOpt < -1){
				EffInfoTree->Fill();
