...
void BsOverBPlus1Bin(){
	...
	TString BsName; 
	if(TNP == 1) BsName = "BsBP1Bin/CrossSectionPbPbBs.root";
	if(TNP == 0) BsName = "BsBP/ReBin/CrossSectionPbPbBsNoTnP.root";


	TString	BPlusName;
	if(TNP == 1) BPlusName = "BsBP1Bin/CrossSectionPbPbBPlus.root";
	if(TNP == 0) BPlusName = "BsBP/ReBin/CrossSectionPbPbBPlusNoTnP.root";
	...
	double BsCorr[_nBins];
	double BPCorr[_nBins];

//	double TnPErrBsUp[_nBins] ={0.143};
//	double TnPErrBsDown[_nBins] ={0.109};

	double TnPErrBsUp[_nBins] ={0.0836};
	double TnPErrBsDown[_nBins] ={0.0737};

	double TnPErrBPUp[_nBins] ={0.0665};
	double TnPErrBPDown[_nBins] ={0.0622};

//	double TnPErrBPUp[_nBins] ={0.0810};
//	double TnPErrBPDown[_nBins] ={0.0720};
...
//	NPartPlot->Draw("epx0");
	...
	double SelErrBs[_nBins] ={0.112};
	double PTErrBs[_nBins] ={0.00012};
	double AccErrBs[_nBins] ={0.0721};
	double PDFBErrBs[_nBins] ={0.0218};
	double PDFSErrBs[_nBins] ={0.0165};
	double MCStatBs[_nBins] ={0.152};
	double ClosureUp[_nBins] ={0.0};
	double ClosureDown[_nBins] ={0.0};
	*/
	...
	double TrackingErrBP[_nBins] ={0.05};
	double SelErrBP[_nBins] ={0.10498};
	double PTErrBP[_nBins] ={0.00139};
	double AccErrBP[_nBins] ={0.0006};
//	double PDFBErrBP[_nBins] ={0.0};
//	double PDFSErrBP[_nBins] ={0.0281};
	double PDFBErrBP[_nBins] ={0.00427};
	double PDFSErrBP[_nBins] ={0.0260};

//	double MCStatBP[_nBins] ={0.0571};
	double MCStatBP[_nBins] ={0.0228};
	double sPLOTBP[_nBins] = {0.0059};

	double TrackingErrBsBP[_nBins] ={0.05};
	...
	ofstream outratio("OutText/Ratio/ratio_cent_0_90_New.txt");
	...
	TLatex* texcent = new TLatex(0.26,0.79, "B |y| < 2.4");
	texcent->SetNDC();
	...
	TLatex *texY2 = new TLatex(0.23,0.74,"10 GeV/c < B p_{T} < 50 GeV/c");
	texY2->SetNDC();
...
*/
	...
	TLatex* Bin1 = new TLatex(0.45,0.70,"Cent. 0 - 90%");
	Bin1->SetNDC();
	...
	TLegend* leg = new TLegend(0.65,0.45,0.85,0.67,NULL,"brNDC");
	leg->SetBorderSize(0);
	...
	if(TNP == 1) c->SaveAs("BstoBPlusCent_0_90_ReBinAdd.pdf");
	if(TNP == 0) c->SaveAs("BstoBPlusCent_0_90_NoTNP_ReBinAdd.pdf");
