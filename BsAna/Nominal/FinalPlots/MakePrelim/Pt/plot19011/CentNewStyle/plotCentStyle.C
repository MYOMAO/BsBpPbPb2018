	...
	//	TCanvas *pc1 = new TCanvas("pc1","pc1",1200,600);
		...
		for(int q = 1; q > -1; q--){
			...
			TGraphAsymmErrors *pgRatio_syst_low = new TGraphAsymmErrors(nBinsLow, binLow, bs_low,
					bs_low_xErrL, bs_low_xErrH,
					bs_low_ySystL,bs_low_ySystH);
			if(q == 1) pgRatio_syst_low  = new TGraphAsymmErrors(nBinsLow, binLow, bs_low,
					bs_low_xErrL2, bs_low_xErrH2,
					bs_low_ySystL,bs_low_ySystH);
			...
			*/
			...
			if(whichPlot==1) f4->GetYaxis()->SetRangeUser(0.0,1.7);
			...
			if(whichPlot==0){
			   ...
			   // legend
				...
				if(q==0) lat->DrawLatex(legXsec_xLowStart,legXsec_y,"#bf{B_{s}^{0}     B^{+}}");
				TLegend *legXSec = new TLegend(legXsec_xLowStart+0.01,legXsec_y-0.05,legXsec_xLowEnd+0.18,legXsec_y+0.05,"   ","brNDC");
				legXSec->SetBorderSize(0);
			...
			//	lat->SetTextSize(ltxSetTextSize1*0.8);
				...
				lat->SetTextFont(42);
				lat->SetTextSize(ltxSetTextSize2);
				if(q==0)	lat->DrawLatex(ratio_ltxText2_xStart-0.02,ratio_ltxText2_yStart,"#splitline{|y| < 2.4}{10 < p_{T} < 50 GeV/c}");

				lat->SetTextFont(42);
				lat->SetTextSize(ltxSetTextSize2);
				if(q==0)	lat->DrawLatex(legRatio_xLowStart-0.08,legRatio_y-0.23,Form("Global uncert.: #pm %.1f %%",glbSystDown));
			...
			//		TLegend *legRatioRef = new TLegend(legRatioRef_xLowStart-0.15,legRatioRef_y-0.05,legRatioRef_xLowEnd-0.15,legRatio_y+0.05,"f_{s}/f_{u} reference","brNDC");
				//	TLegend *legRatioRef = new TLegend(legRatioRef_xLowStart,legRatioRef_y,legRatioRef_xLowEnd,legRatio_y+0.1,"LHCb pp 7TeV (arXiv.:1301.5286)","brNDC");
					TLegend *legRatioRef = new TLegend(legRatioRef_xLowStart-0.15,legRatioRef_y-0.10,legRatioRef_xLowEnd-0.15,legRatio_y+0.00,"","brNDC");
