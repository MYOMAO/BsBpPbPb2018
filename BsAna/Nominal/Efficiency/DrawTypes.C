...
void DrawTypes(int CentMin, int CentMax, int Option){
	...
	TString infile = Form("NewTnPScheme/EffInfo_%d_%d.root",CentMin,CentMax);
	...
	lumiDiff->Draw();
	c->SaveAs("Plots/EvtInfo/lumiDiff.png");
	...
	runDiff->Draw();
	c->SaveAs("Plots/EvtInfo/runDiff.png");
	...
	evtDiff->Draw();
	c->SaveAs("Plots/EvtInfo/evtDiff.png");
