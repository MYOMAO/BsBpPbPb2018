...
void ReAnaEffL2L3(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int drop, int Weight ){
	...
	FileName = Form("/export/d00/scratch/zzshi/CMSSW_7_5_8_patch3/Merge/L2L3Muons/Bs/FinalFiles/Bs/EffInfo_%d_%d.root",CentMin,CentMax);
...
//	TFile * finTnP =  new TFile(Form("OldScheme/TNP2D_Bs_Cent_detailed%d-%d.root",CentMin,CentMax));
	...
	//TFile * finTnP =  new TFile(Form("TnPL2L3/TNP2D_Bs_Cent_detailed%d-%d.root",CentMin,CentMax));
	TFile * finTnPOld =  new TFile(Form("OldTnP/TNP2D_Bs_Cent%d-%d.root",CentMin,CentMax));
	...
	finTnPOld->cd();
	TH2D * tnp_scaleOld = (TH2D *) finTnPOld->Get("tnp_scale");
	TH2D * tnp_total_dOld = (TH2D *) finTnPOld->Get("tnp_total_d");
	TH2D * tnp_total_uOld = (TH2D *) finTnPOld->Get("tnp_total_u");
	...
	for( int i = 0; i < NEvents; i++){
		...
		for(int j = 0; j < BsizeNew; j++){
			...
			for(int k = 0; k < NBins; k++){
				...
				if(BptNew[j] > ptBins[k] && BptNew[j] < ptBins[k+1] && (( BptNew[j] < 10 && TMath::Abs(BmassNew[j] - 5.36682) < 0.09) || (BptNew[j] > 10 && TMath::Abs(BmassNew[j] - 5.36682) < 0.08)) && (Bmu1Type > -0.1 && Bmu2Type > -0.1) )
				{
			...
			//		cout << "Candidate: " << "   Bpt = " << BptNew[j] << "   Efficiency =  " << BEffInv[j] << "   Efficiency Error  = " << BEffInvErr[j] << endl;
				...
				//			cout<< "tnp_scaleOld = " << tnp_scaleOld->GetBinContent(PtBin,EtaBin)  << "   tnp_scale = " <<   tnp_scale->GetBinContent(PtBin,EtaBin) << "  tnp_scale0 =  " << tnp_scale0->GetBinContent(PtBin,EtaBin)  <<  "   tnp_scale1 =  " << tnp_scale1->GetBinContent(PtBin,EtaBin) <<   "   tnp_scale2 =  " << tnp_scale2->GetBinContent(PtBin,EtaBin) <<  "   tnp_scale3 =  " << tnp_scale3->GetBinContent(PtBin,EtaBin) <<  "   tnp_scale4 =  " << tnp_scale4->GetBinContent(PtBin,EtaBin) << endl;
						...
						if(DoLater == 3){
							...
							EtaBin = tnp_scaleOld->GetYaxis()->FindBin(ByNew[j]);
							PtBin = tnp_scaleOld->GetXaxis()->FindBin(BptNew[j]);
							BEffInv[j] =  BEffInv[j]/tnp_scaleOld->GetBinContent(PtBin,EtaBin);
							BEffInvErr[j] = BEffInvErr[j]/tnp_scaleOld->GetBinContent(PtBin,EtaBin);
							BEffInvUp[j] = BEffInv[j]/(1-tnp_total_dOld->GetBinContent(PtBin,EtaBin));
							BEffInvDown[j] = BEffInv[j]/(1+tnp_total_uOld->GetBinContent(PtBin,EtaBin));
							BEffInvErrUp[j] = BEffInvErr[j]/(1-tnp_total_dOld->GetBinContent(PtBin,EtaBin));
							BEffInvErrDown[j] = BEffInvErr[j]/(1+tnp_total_uOld->GetBinContent(PtBin,EtaBin));
...
//		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("CheckSystNuno/NoWeight/MCSPlit/%d-%d/MCRemerged_0_90.root",CentMin,CentMax));
