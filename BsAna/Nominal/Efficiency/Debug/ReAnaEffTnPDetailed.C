...
void ReAnaEffTnPDetailed(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int drop, int Weight, int TnPOpt ){
	...
	TString TnPTag;
	int DoLater = 1;
	if(TnPOpt == 0) TnPTag = "muid";
	if(TnPOpt == 1) TnPTag = "trg";
	if(TnPOpt == 2) TnPTag = "trk";

	TFile * finTnP =  new TFile(Form("NewTnP/TNP2D_Bplus_Cent_detailed%d-%d.root",CentMin,CentMax));
	finTnP->cd();
	TH2D * tnp_scale = (TH2D *) finTnP->Get(Form("tnp_%s_scal",TnPTag.Data()));
	TH2D * tnp_total_d = (TH2D *) finTnP->Get(Form("tnp_%s_d",TnPTag.Data()));
	TH2D * tnp_total_u = (TH2D *) finTnP->Get(Form("tnp_%s_u",TnPTag.Data()));
	...
	if(CentMin == 0 && CentMax == 90 && NBins == 1){
		hInvEff->SetMinimum(30);
		hInvEff->SetMaximum(50);
	}

	if(CentMin == 0 && CentMax == 30 && NBins == 1){
		hInvEff->SetMinimum(40);
		hInvEff->SetMaximum(55);
	}
	if(CentMin == 30 && CentMax == 90 && NBins == 1){
		hInvEff->SetMinimum(10);
		hInvEff->SetMaximum(20);
	}
	if(CentMin == 0 && CentMax == 90 && NBins == 4){
		hInvEff->SetMinimum(0);
		hInvEff->SetMaximum(620);
	}
	...
	TLegend *legTnP = new TLegend(0.17,0.40,0.44,0.67,NULL,"brNDC");
	legTnP->SetBorderSize(0);
	legTnP->SetTextSize(0.04);
	legTnP->SetTextFont(42);
	legTnP->SetFillStyle(0);

	legTnP->AddEntry(hInvEff,Form("TnP - %s: Nominal Correction",TnPTag.Data()),"pl");
	legTnP->AddEntry(hInvEffUp,Form("TnP - %s: Upper Bound of Scale Factor",TnPTag.Data()),"pl");
	legTnP->AddEntry(hInvEffDown,Form("TnP - %s: Lower Bound of Scale Factor",TnPTag.Data()),"pl");
	legTnP->Draw("SAME");



//	c->SaveAs(Form("CheckSystNuno/%s/TnPFinal/TnPEffComp_%dBins_%d_%d.png",WeightName.Data(),NBins,CentMin,CentMax));
	c->SaveAs(Form("CheckSystNuno/%s/TnPFinal/%s/TnPEffComp_%dBins_%d_%d.png",WeightName.Data(),TnPTag.Data(),NBins,CentMin,CentMax));
	...
	TLegend *legTnPRatio = new TLegend(0.25,0.30,0.59,0.45,NULL,"brNDC");
	legTnPRatio->SetBorderSize(0);
	legTnPRatio->SetTextSize(0.04);
	legTnPRatio->SetTextFont(42);
	legTnPRatio->SetFillStyle(0);

	legTnPRatio->AddEntry(hInvUpRatio,Form("TnP - %s Up/Nominal",TnPTag.Data()),"pl");
	legTnPRatio->AddEntry(hInvDownRatio,Form("TnP - %s Down/Nominal",TnPTag.Data()),"pl");
...
//	c->SaveAs(Form("CheckSystNuno/%s/TnPFinal/SystTnPRatio_%dBins_%d_%d.png",WeightName.Data(),NBins,CentMin,CentMax));
	c->SaveAs(Form("CheckSystNuno/%s/TnPFinal/%s/SystTnPRatio_%dBins_%d_%d.png",WeightName.Data(),TnPTag.Data(),NBins,CentMin,CentMax));
