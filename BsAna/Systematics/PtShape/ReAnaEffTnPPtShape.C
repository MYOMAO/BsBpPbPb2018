...
void ReAnaEffTnPPtShape(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int drop, int Weight){
	...
	FileName = Form("CheckSystNuno/NoTnP/EffInfo_%d_%d.root",CentMin,CentMax);


	TFile * finMap = new TFile(Form("CheckSystNuno/%s/EffFine_%d_%d.root",WeightName.Data(),CentMin,CentMax));
	finMap->cd();

	TH2D * hEff2DInv2Shots = (TH2D * ) finMap->Get("hEff2DInv2Shots");
		...
		for(int j = 0; j < BsizeNew; j++){
			...
			for(int k = 0; k < NBins; k++){
				...
				if(BptNew[j] > ptBins[k] && BptNew[j] < ptBins[k+1] && (( BptNew[j] < 10 && TMath::Abs(BmassNew[j] - 5.36682) < 0.09) || (BptNew[j] > 10 && TMath::Abs(BmassNew[j] - 5.36682) < 0.08)) && (Bmu1Type[j] > -0.1&& Bmu2Type[j] > -0.1)){
					...
					PtBin = hEff2DInv2Shots->GetXaxis()->FindBin(BptNew[j]);
					EtaBin = hEff2DInv2Shots->GetYaxis()->FindBin(ByNew[j]);


					BEffInv[j] = hEff2DInv2Shots->GetBinContent(PtBin,EtaBin);
					BEffInvErr[j] = hEff2DInv2Shots->GetBinError(PtBin,EtaBin);

					BEffInvUp[j] = hEff2DInv2Shots->GetBinContent(PtBin,EtaBin);
					BEffInvErrUp[j] = hEff2DInv2Shots->GetBinError(PtBin,EtaBin);

					BEffInvDown[j] = hEff2DInv2Shots->GetBinContent(PtBin,EtaBin);
					BEffInvErrDown[j] = hEff2DInv2Shots->GetBinError(PtBin,EtaBin);
	...
	if(CentMin ==0 && CentMax==90){
		TAA = 6.274; 
		if(NBins == 1 && drop == 0)		RawYield = new TFile("RawYieldsNew/yields_Bs_full_0_90_750.root");
