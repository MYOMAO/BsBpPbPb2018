...
void roofitB(int doubly = 0, TString tree = "ntphi", int full = 1, int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString _varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 1, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100., TString PDFVAR = "", TString VarFunc = "")
{
	...
	for(int i=0;i<nBins_check;i++)
	{
		...
		RooFitResult* f = fit(PDFVAR, VarFunc, tree, c, cMC, ds_cut, dsMC_cut, dh, dhMC, mass, frame, ptBins_check[i], ptBins_check[i+1], isMC, isPbPb, centmin, centmax, npfit);
	...
	//	validate_fit(w_val, tree, _varExp, full,centmin, centmax, ptBins_check[i], ptBins_check[i+1]);
		...
		*/
		...
		std::ofstream OutPDFFile(Form("OutTexts/PDF%.0f_%.0f_%.0f_%.0f.dat",centmin,centmax,ptBins_check[i],ptBins_check[i+1]), ios::app);
		OutPDFFile << "PDFVAR: " << PDFVAR.Data() <<  "   VarFunc: " << VarFunc.Data() << "     Yield: " << yieldRec << std::endl;
		...
		if(f->floatParsFinal().index(Form("nsig%d",_count)) != -1 && VarFunc != "fixed"){
		...
		c->SaveAs(Form("%s%s/%s_%s_%d%s_%s_%d%d_doubly%d_%.0f_%0.f_%s_%s",outplotf.Data(),_prefix.Data(),_isMC.Data(),_isPbPb.Data(),_count,_postfix.Data(),_varExp.Data(),(int)ptBins_check[i],(int)ptBins_check[i+1], doubly,centmin,centmax,PDFVAR.Data(),VarFunc.Data())+tree+".pdf");
		c->Update();
		c->SaveAs(Form("%s%s/%s_%s_%d%s_%s_%d%d_doubly%d_%.0f_%0.f_%s_%s",outplotf.Data(),_prefix.Data(),_isMC.Data(),_isPbPb.Data(),_count,_postfix.Data(),_varExp.Data(),(int)ptBins_check[i],(int)ptBins_check[i+1], doubly,centmin,centmax,PDFVAR.Data(),VarFunc.Data())+tree+".png");
		...
		cMC->SaveAs(Form("%s%s/%s_%s_%d%s_%s_%d%d_doubly%d_%.0f_%0.f_%s_%s",outplotf.Data(),_prefix.Data(),"mc",_isPbPb.Data(),_count,_postfix.Data(),_varExp.Data(), (int)ptBins_check[i], (int)ptBins_check[i+1], doubly,centmin,centmax,PDFVAR.Data(),VarFunc.Data())+tree+".pdf");
		cMC->SaveAs(Form("%s%s/%s_%s_%d%s_%s_%d%d_doubly%d_%.0f_%0.f_%s_%s",outplotf.Data(),_prefix.Data(),"mc",_isPbPb.Data(),_count,_postfix.Data(),_varExp.Data(), (int)ptBins_check[i], (int)ptBins_check[i+1], doubly,centmin,centmax,PDFVAR.Data(),VarFunc.Data())+tree+".png");
