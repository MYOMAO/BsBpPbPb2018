...
void roofitB(int doubly = 0, TString tree = "ntphi", int full = 1, int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString _varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 1, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100.)
{
	...
	RooRealVar* DataWeight_5_10 = new RooRealVar("DataWeight_5_10","DataWeight_5_10",0.5,1.5) ;
	RooRealVar* DataWeight_10_15 = new RooRealVar("DataWeight_10_15","DataWeight_10_15",0.5,1.5) ;
	RooRealVar* DataWeight_15_20 = new RooRealVar("DataWeight_15_20","DataWeight_15_20",0.5,1.5) ;
	RooRealVar* DataWeight_20_50 = new RooRealVar("DataWeight_20_50","DataWeight_20_50",0.5,1.5) ;
	...
	TCut JustCut = "(BptNew > 0)";
	TCut WeightMe = "(DataWeight_10_15)";

	TTree *	skimtree_new_weighted = (TTree*) skimtree_new->CopyTree(TCut(JustCut)*TCut(WeightMe),"skimtree_new_weighted");

	//	ds = new RooDataSet(Form("ds%d",_count),"",skimtree_new, RooArgSet(*mass, *pt, *HiBin, *y, *DataWeight_5_10, * DataWeight_10_15, * DataWeight_15_20, *DataWeight_20_50));
	ds = new RooDataSet(Form("ds%d",_count),"",skimtree_new_weighted, RooArgSet(*mass, *pt, *HiBin, *y, *DataWeight_5_10, *DataWeight_10_15, *DataWeight_15_20, *DataWeight_20_50));
	...
	for(int i=0;i<nBins_check;i++)
	{
		...
		if(ptBins_check[i] == 7) PtIndex = 0;
		if(ptBins_check[i] == 10) PtIndex = 1;
		if(ptBins_check[i] == 15) PtIndex = 2;
		if(ptBins_check[i] == 20) PtIndex = 3;
		if(ptBins_check[i] == 10 && ptBins_check[i+1] == 50 ) PtIndex = 4;

		/*
		   RooFormulaVar wFunc0("DataWeight_5_10","event weight 0","DataWeight_5_10", *DataWeight_5_10) ;
		   RooRealVar* DataWeight_5_10_Use = (RooRealVar*) ds->addColumn(wFunc0) ;
		   RooDataSet weighteddata0(ds->GetName(),ds->GetTitle(),ds,*ds->get(),0,DataWeight_5_10_Use->GetName()) ;
		   weighteddata0.add(RooArgSet(*mass, *pt, *HiBin, *y));

		   RooFormulaVar wFunc1("DataWeight_10_15","event weight 1","DataWeight_10_15", *DataWeight_10_15) ;
		   RooRealVar* DataWeight_10_15_Use = (RooRealVar*) ds->addColumn(wFunc1) ;
		   RooDataSet weighteddata1(ds->GetName(),ds->GetTitle(),ds,*ds->get(),0,DataWeight_10_15_Use->GetName()) ;
		   weighteddata1.add(RooArgSet(*mass, *pt, *HiBin, *y));

		   RooFormulaVar wFunc2("DataWeight_15_20","event weight 2","DataWeight_15_20", *DataWeight_15_20) ;
		   RooRealVar* DataWeight_15_20_Use = (RooRealVar*) ds->addColumn(wFunc2) ;
		   RooDataSet weighteddata2(ds->GetName(),ds->GetTitle(),ds,*ds->get(),0,DataWeight_15_20_Use->GetName()) ;
		   weighteddata2.add(RooArgSet(*mass, *pt, *HiBin, *y));

		   RooFormulaVar wFunc3("DataWeight_20_50","event weight 3","DataWeight_20_50", *DataWeight_20_50) ;
		   RooRealVar* DataWeight_20_50_Use = (RooRealVar*) ds->addColumn(wFunc3) ;
		   RooDataSet weighteddata3(ds->GetName(),ds->GetTitle(),ds,*ds->get(),0,DataWeight_20_50_Use->GetName()) ;
		   weighteddata3.add(RooArgSet(*mass, *pt, *HiBin, *y));
		   */
		...
		/*
		   ...
		   if(doubly==0 && PtIndex == 0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata0, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 
		   if(doubly==0 && PtIndex == 1)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata1, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 
		   if(doubly==0 && PtIndex == 2)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata2, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 
		   if(doubly==0 && PtIndex == 3)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata3, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 
		   */

		if(doubly==0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y, *DataWeight_5_10, *DataWeight_10_15, *DataWeight_15_20, *DataWeight_20_50), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&hiBinNew>=%f&&hiBinNew<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto, centmin * 2, centmax * 2)); 


		RooFormulaVar * wFunc;
		if(PtIndex == 0) wFunc = new RooFormulaVar("RatioWeight","event weight 0","(DataWeight_5_10)",RooArgSet(*DataWeight_5_10));
		if(PtIndex == 1) wFunc = new RooFormulaVar("RatioWeight","event weight 1","(DataWeight_10_15)", RooArgSet(*DataWeight_10_15));
		if(PtIndex == 2) wFunc = new RooFormulaVar("RatioWeight","event weight 2","(DataWeight_15_20)",RooArgSet(*DataWeight_15_20));
		if(PtIndex == 3) wFunc = new RooFormulaVar("RatioWeight","event weight 3","(DataWeight_20_50)",RooArgSet(*DataWeight_20_50));
		if(PtIndex == 4) wFunc = new RooFormulaVar("RatioWeight","event weight 4","(DataWeight_10_15* DataWeight_15_20 * DataWeight_20_50)",RooArgSet(*DataWeight_10_15, *DataWeight_15_20, *DataWeight_20_50));

		//RooFormulaVar wFunc("RatioWeight","event weight 0","(DataWeight_10_15)",RooArgSet(*DataWeight_10_15));

		RooRealVar * DataWeightUse = (RooRealVar*) ds_cut->addColumn(*wFunc);
		RooDataSet weighteddata(ds_cut->GetName(),ds_cut->GetTitle(),ds_cut,*ds_cut->get(),0,DataWeightUse->GetName()) ;
		...
		//		if(doubly==1)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&HiBin>=0&&HiBin<=60",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		//		if(doubly==2)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",&weighteddata, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f&&HiBin>=60&&HiBin<=180",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		...
		TCut CutonMC = Form("%s*(%s&&%s>%f&&%s<%f && BgenNew == 23333)*(1/%s)",weightmc.Data(),seldata.Data(),varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],weightdata.Data());
		TCut CutonData = Form("(%s&&%s>%f&&%s<%f)",                seldata.Data(),varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1]);
		TCut WeightonCut;

		if(PtIndex == 0) WeightonCut = "(DataWeight_5_10)";
		if(PtIndex == 1) WeightonCut = "(DataWeight_10_15)";
		if(PtIndex == 2) WeightonCut = "(DataWeight_15_20)";
		if(PtIndex == 3) WeightonCut = "(DataWeight_20_50)";
		if(PtIndex == 4) WeightonCut = "((DataWeight_20_50)*(DataWeight_15_20)*(DataWeight_10_15)*(DataWeight_5_10))";



				
		skimtree_new->Project(Form("h%d",_count),"BmassNew", TCut(CutonData)*TCut(WeightonCut));
		WeightedCand = h->Integral();
			
		skimtree_new->Project(Form("h%d",_count),"BmassNew",TCut(CutonData));
		UnweightedCand = h->Integral();


		RelScale = UnweightedCand/WeightedCand;


		if(isMC==1) skimtree_new->Project(Form("h%d",_count),"BmassNew",TCut(CutonMC) );
		else        skimtree_new->Project(Form("h%d",_count),"BmassNew", TCut(CutonData)*TCut(WeightonCut));

		cout << "----------------------------------------------------------------------------------------------" << endl;
		cout << "His ROOFIT h Integral  " << h->Integral() << endl;
		cout << "----------------------------------------------------------------------------------------------" << endl;
		...
		/*
		   if(PtIndex == 0) outputw->import(weighteddata0);
		   if(PtIndex == 1) outputw->import(weighteddata1);
		   if(PtIndex == 2) outputw->import(weighteddata2);
		   if(PtIndex == 3) outputw->import(weighteddata3);
		   */

		outputw->import(weighteddata);
		outputw->import(*dsMC);
		outputw->import(*dh);
		outputw->import(*dhMC);
		...
		RooFitResult* f = fit("", "", tree, c, cMC, &weighteddata, dsMC_cut, dh, dhMC, mass, frame, ptBins_check[i], ptBins_check[i+1], isMC, isPbPb, centmin, centmax, npfit);
	...
	cout << "Final Yield Before = " << yieldRec << endl;

	double FinalYield =  yieldRec * RelScale;

	cout << "Final Yield Compare = " << FinalYield << endl;
