...
void SpiltDataBoot(int CentMin, int CentMax, int PtOpt){
	...
	*/
	...
	infile = Form("CheckSystNuno/%d-%d/%s/EffInfo.root",CentMin,CentMax,outfilefolder.Data());
...
//	int NEvents = EffInfoTree->GetEntries();
	int NEvents;
	if(CentMin == 0 && CentMax ==90 &&PtOpt == 0) NEvents = 7;
	if(CentMin == 0 && CentMax ==90 &&PtOpt == 3) NEvents = 31;
	if(CentMin == 0 && CentMax ==90 &&PtOpt == 1) NEvents = 29;
	if(CentMin == 0 && CentMax ==90 &&PtOpt == 2) NEvents = 28;
	if(CentMin == 0 && CentMax ==90 &&PtOpt == -1) NEvents = 100;

	if(CentMin == 0 && CentMax ==30 &&PtOpt == -1) NEvents = 59;
	if(CentMin == 30 && CentMax == 90 &&PtOpt == -1) NEvents = 30;
...
//	cout << "NEvents = " << NEvents << endl;
	...
	TF1 *f1 = new TF1("f1",f1Name.Data(),0,1000);
