...
void roofitB(int doubly = 0, TString tree = "ntphi", int full = 1, int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString _varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 1, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100.)
{
	...
	RooRealVar* bdt_pt_5_10 = new RooRealVar("BDT_pt_5_10New","BDT_pt_5_10New",-1,1);
	RooRealVar* bdt_pt_10_15 = new RooRealVar("BDT_pt_10_15New","BDT_pt_10_15New",-1,1);
	RooRealVar* bdt_pt_15_20 = new RooRealVar("BDT_pt_15_20New","BDT_pt_15_20New",-1,1);
	RooRealVar* bdt_pt_20_50 = new RooRealVar("BDT_pt_20_50New","BDT_pt_20_50New",-1,1);
	...
	ds = new RooDataSet(Form("ds%d",_count),"",skimtree_new, RooArgSet(*mass, *pt, *HiBin, *y, *bdt_pt_5_10, *bdt_pt_10_15, *bdt_pt_15_20, *bdt_pt_20_50));
	dsMC = new RooDataSet(Form("dsMC%d",_count),"",skimtreeMC_new,RooArgSet(*mass, *pt, *w, *HiBin, *y, *bdt_pt_5_10, *bdt_pt_10_15, *bdt_pt_15_20, *bdt_pt_20_50));
	//    TString ptbinning;
	...
	cout << "FUCKING: seldata = " << seldata.Data() << endl;
	...
	for(int i=0;i<nBins_check;i++)
	{
		...
		if(doubly==0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y, *bdt_pt_5_10, *bdt_pt_10_15, *bdt_pt_15_20, *bdt_pt_20_50), Form("(%s)&&(%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f)",cut.Data(),varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		...
		RooDataSet* dsMC_cut;
		if(doubly==0) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *HiBin, *y, *bdt_pt_5_10, *bdt_pt_10_15, *bdt_pt_15_20, *bdt_pt_20_50), Form("(%s)&&(%s>=%f&&%s<=%f&&BmassNew>%f&&BmassNew<%f)",cut.Data(),varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "pthatweightNew"); 
