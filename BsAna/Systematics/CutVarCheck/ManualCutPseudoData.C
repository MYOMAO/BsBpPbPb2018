...
void ManualCutPseudoData(int CentMin, int CentMax, int IsMC, int Shape, int TwoShot, int PtOpt, int pthatCut, int TEffOpt){
	...
	if(pthatCut == -1){
		ptahtCutValueDown = 0;
		ptahtCutValueUp = 1000;
	}


	if(pthatCut == 0){
		ptahtCutValueDown = 0.00;
		ptahtCutValueUp = 0.05;

	}

	if(pthatCut == 1){
		ptahtCutValueDown = 0.05;
		ptahtCutValueUp = 0.35;

	}

	if(pthatCut == 2){
		ptahtCutValueDown = 0.35;
		ptahtCutValueUp = 1.6;

	}

	if(pthatCut == 3){
		ptahtCutValueDown = 1.6;
		ptahtCutValueUp = 5.8;
	...
	TFile * finEffTEff = new TFile(Form("CheckSystNuno/%s/EffFine_%d_%dTEff.root",FolderName.Data(),CentMin,CentMax));
	finEffTEff->cd();

	TEfficiency * TEff = (TEfficiency *) finEffTEff->Get("EffBptByTEff");
	...
	if(IsMC == 1) fout = new TFile(Form("New/%d-%d/%s/EffInfo_MC_Closure%d.root",CentMin,CentMax,outfilept.Data(),pthatCut),"RECREATE");
	...
	EffInfoTree->Branch("pthatweightNew",&pthatweightNew,"pthatweightNew/F");
	...
	Double_t BDT_pt_5_10New[NCand];
	Double_t BDT_pt_10_15New[NCand];
	Double_t BDT_pt_15_20New[NCand];
	Double_t BDT_pt_20_50New[NCand];

	EffInfoTree->Branch("BDT_pt_5_10New",BDT_pt_5_10New,"BDT_pt_5_10New/D");
	EffInfoTree->Branch("BDT_pt_10_15New",BDT_pt_10_15New,"BDT_pt_10_15New/D");
	EffInfoTree->Branch("BDT_pt_15_20New",BDT_pt_15_20New,"BDT_pt_15_20New/D");
	EffInfoTree->Branch("BDT_pt_20_50New",BDT_pt_20_50New,"BDT_pt_20_50New/D");
	...
	EffInfoTree->Branch("NGenCand",&NGenCand,"NGenCand/I");
	...
	for(int i = 0; i < NEvents; i++){
		...
		for(int j =0; j < Bsize; j++){
			...
			if((HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1 == 1) && (Bpt[j] > BptLow && Bpt[j] < BptHigh) &&(pthatweight > ptahtCutValueDown && pthatweight < ptahtCutValueUp) && (((hiBin < 181) && Btrk1Pt[j] > 0.9 && Btrk2Pt[j] > 0.9 && Bchi2cl[j] > 0.05 && BsvpvDistance[j]/BsvpvDisErr[j] > 2.0 && Bpt[j] > 5 && abs(Btrk1Eta[j]-0.0) < 2.4 && abs(Btrk2Eta[j]-0.0) < 2.4 && (TMath::Abs(By[j])<2.4&&TMath::Abs(Bmumumass[j]-3.096916)<0.15&&((abs(Bmu1eta[j])<1.2&&Bmu1pt[j]>3.5)||(abs(Bmu1eta[j])>1.2&&abs(Bmu1eta[j])<2.1&&Bmu1pt[j]>(5.47-1.89*abs(Bmu1eta[j])))||(abs(Bmu1eta[j])>2.1&&abs(Bmu1eta[j])<2.4&&Bmu1pt[j]>1.5))&&((abs(Bmu2eta[j])<1.2&&Bmu2pt[j]>3.5)||(abs(Bmu2eta[j])>1.2&&abs(Bmu2eta[j])<2.1&&Bmu2pt[j]>(5.47-1.89*abs(Bmu2eta[j])))||(abs(Bmu2eta[j])>2.1&&abs(Bmu2eta[j])<2.4&&Bmu2pt[j]>1.5))&&Bmu1TMOneStationTight[j]&&Bmu2TMOneStationTight[j]&&Bmu1InPixelLayer[j]>0&&(Bmu1InPixelLayer[j]+Bmu1InStripLayer[j])>5&&Bmu2InPixelLayer[j]>0&&(Bmu2InPixelLayer[j]+Bmu2InStripLayer[j])>5&&Bmu1dxyPV[j]<0.3&&Bmu2dxyPV[j]<0.3&&Bmu1dzPV[j]<20&&Bmu2dzPV[j]<20&&Bmu1isTrackerMuon[j]&&Bmu2isTrackerMuon[j]&&Bmu1isGlobalMuon[j]&&Bmu2isGlobalMuon[j]&&Btrk1highPurity[j]&&Btrk2highPurity[j]&&abs(Btrk1Eta[j])<2.4&&abs(Btrk2Eta[j])<2.4&&Btrk1Pt[j]>1.&&Btrk2Pt[j]>1.&&abs(Btktkmass[j]-1.019455)<0.015) && (abs(PVz)<15&&pclusterCompatibilityFilter == 1 &&pprimaryVertexFilter == 1) && (Btrk1PixelHit[j] + Btrk1StripHit[j] > 10) && (Btrk2PixelHit[j] + Btrk2StripHit[j] > 10) && (Btrk1PtErr[j]/Btrk1Pt[j] < 0.1)&& (Btrk2PtErr[j]/Btrk2Pt[j] < 0.1) && Btrk1Chi2ndf[j]/(Btrk1nStripLayer[j]+Btrk1nPixelLayer[j]) < 0.18 && Btrk2Chi2ndf[j]/(Btrk2nStripLayer[j]+Btrk2nPixelLayer[j]) < 0.18&&((Bpt[j]>5&&Bpt[j]<10&&BDT_pt_5_10[j]>0.0 )||(Bpt[j]>10&&Bpt[j]<15&&BDT_pt_10_15[j]> 0.0)||(Bpt[j]>15&&Bpt[j]<20&&BDT_pt_15_20[j]>0.0)||(Bpt[j]>20&&Bpt[j]<50&&BDT_pt_20_50[j]>0.0))&&abs(PVz)<15&&pclusterCompatibilityFilter == 1 &&pprimaryVertexFilter == 1 && phfCoincFilter2Th4 == 1) && (Bmass[j] > 5 &&Bmass[j] < 6 && hiBin >= CentMin * 2&& hiBin <= CentMax * 2 && Bgen[j] == 23333 )) ){
				...
				//	if( (HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1 == 1) && (Bpt[j] > BptLow && Bpt[j] < BptHigh) &&(pthatweight > ptahtCutValueDown && pthatweight < ptahtCutValueUp) && (((hiBin < 181) && Btrk1Pt[j] > 0.9 && Btrk2Pt[j] > 0.9 && Bchi2cl[j] > 0.05 && BsvpvDistance[j]/BsvpvDisErr[j] > 2.0 && Bpt[j] > 5 && abs(Btrk1Eta[j]-0.0) < 2.4 && abs(Btrk2Eta[j]-0.0) < 2.4 && (TMath::Abs(By[j])<2.4&&TMath::Abs(Bmumumass[j]-3.096916)<0.15&&((abs(Bmu1eta[j])<1.2&&Bmu1pt[j]>3.5)||(abs(Bmu1eta[j])>1.2&&abs(Bmu1eta[j])<2.1&&Bmu1pt[j]>(5.47-1.89*abs(Bmu1eta[j])))||(abs(Bmu1eta[j])>2.1&&abs(Bmu1eta[j])<2.4&&Bmu1pt[j]>1.5))&&((abs(Bmu2eta[j])<1.2&&Bmu2pt[j]>3.5)||(abs(Bmu2eta[j])>1.2&&abs(Bmu2eta[j])<2.1&&Bmu2pt[j]>(5.47-1.89*abs(Bmu2eta[j])))||(abs(Bmu2eta[j])>2.1&&abs(Bmu2eta[j])<2.4&&Bmu2pt[j]>1.5))&&Bmu1TMOneStationTight[j]&&Bmu2TMOneStationTight[j]&&Bmu1InPixelLayer[j]>0&&(Bmu1InPixelLayer[j]+Bmu1InStripLayer[j])>5&&Bmu2InPixelLayer[j]>0&&(Bmu2InPixelLayer[j]+Bmu2InStripLayer[j])>5&&Bmu1dxyPV[j]<0.3&&Bmu2dxyPV[j]<0.3&&Bmu1dzPV[j]<20&&Bmu2dzPV[j]<20&&Bmu1isTrackerMuon[j]&&Bmu2isTrackerMuon[j]&&Bmu1isGlobalMuon[j]&&Bmu2isGlobalMuon[j]&&Btrk1highPurity[j]&&Btrk2highPurity[j]&&abs(Btrk1Eta[j])<2.4&&abs(Btrk2Eta[j])<2.4&&Btrk1Pt[j]>1.&&Btrk2Pt[j]>1.&&abs(Btktkmass[j]-1.019455)<0.015) && (abs(PVz)<15&&pclusterCompatibilityFilter == 1 &&pprimaryVertexFilter == 1) && (Btrk1PixelHit[j] + Btrk1StripHit[j] > 10) && (Btrk2PixelHit[j] + Btrk2StripHit[j] > 10) && (Btrk1PtErr[j]/Btrk1Pt[j] < 0.1)&& (Btrk2PtErr[j]/Btrk2Pt[j] < 0.1) && Btrk1Chi2ndf[j]/(Btrk1nStripLayer[j]+Btrk1nPixelLayer[j]) < 0.18 && Btrk2Chi2ndf[j]/(Btrk2nStripLayer[j]+Btrk2nPixelLayer[j]) < 0.18&&((Bpt[j]>5&&Bpt[j]<10&&BDT_pt_5_10[j]> -1 )||(Bpt[j]>10&&Bpt[j]<15&&BDT_pt_10_15[j]> -1)||(Bpt[j]>15&&Bpt[j]<20&&BDT_pt_15_20[j]>-1)||(Bpt[j]>20&&Bpt[j]<50&&BDT_pt_20_50[j]>-1))&&abs(PVz)<15&&pclusterCompatibilityFilter == 1 &&pprimaryVertexFilter == 1 && phfCoincFilter2Th4 == 1) && (Bmass[j] > 5 &&Bmass[j] < 6 && hiBin >= CentMin * 2&& hiBin <= CentMax * 2)) ){
				//if((HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1 == 1) && (Bpt[j] > BptLow && Bpt[j] < BptHigh) && (Bmass[j] > 5 &&Bmass[j] < 6 && hiBin >= CentMin * 2&& hiBin <= CentMax * 2 && Bgen[j] == 23333 ))
				//	if(1 > 0){

				pthatweightNew = pthatweight;
				...
				BDT_pt_5_10New[iPass] = BDT_pt_5_10[j];
				BDT_pt_10_15New[iPass] = BDT_pt_10_15[j];
				BDT_pt_15_20New[iPass] = BDT_pt_15_20[j];
				BDT_pt_20_50New[iPass] = BDT_pt_20_50[j];
				...
				if(TEffOpt == 1){
					...
					globalbin = TEff->GetGlobalBin(XLoc,YLoc);
					BEff[iPass] =  TEff->GetEfficiency(globalbin);
					BEffInv[iPass] = 1.00/BEff[iPass];
					BEffInvErr[iPass] =  TEff->GetEfficiencyErrorUp(globalbin)/(BEff[iPass] * BEff[iPass]);
					BEffInvErrUp[iPass] =  TEff->GetEfficiencyErrorUp(globalbin)/(BEff[iPass] * BEff[iPass]);
					BEffInvErrDown[iPass] =  TEff->GetEfficiencyErrorLow(globalbin)/(BEff[iPass] * BEff[iPass]);
			...
			//		if(Gsize > 1) cout << "Gsize = " << Gsize << endl;

			//	cout << "NGenCand = " << NGenCand << endl; 
			...
			for(int k=0; k< Gsize;k++){
				...
				/*
				   ...
				   cout << " ----------------------------- Gen Content ----------------------------- " << endl;

				   cout  <<  "Abs(Gy[k]) = "  << TMath::Abs(Gy[k]) << endl;
				   cout  <<  "Abs(GpdgId[k]) = "  << TMath::Abs(GpdgId[k]) << endl;
				   cout  <<  "Abs(GisSignal[k]) = "  << TMath::Abs(GisSignal[k]) << endl;
				   cout  <<  "Gpt =  " << Gpt[k] << endl;
				...
				if( TMath::Abs(Gy[k])<2.4&&abs(GpdgId[k])==531&&GisSignal[k]>0 && (Gpt[k] > BptLow && Gpt[k] < BptHigh && pthatweight > ptahtCutValueDown && pthatweight < ptahtCutValueUp) && (hiBin >= CentMin * 2&& hiBin <= CentMax * 2 )  &&((Gpt[k] > 7 && Gpt[k] < 10 && abs(Gy[k])>1.5)||(Gpt[k] > 10 && Gpt[k] < 50 && abs(Gy[k])>0))){
