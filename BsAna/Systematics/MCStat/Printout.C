...
void Printout(){
	...
	TFile * fin = new TFile("2DMaps/AccNew.root");
	fin->cd();

	TH2D * hPtGen2DFine = (TH2D * ) fin->Get("hPtGen2DFine");
	TH2D * hPtGenAcc2DFine = (TH2D * ) fin->Get("hPtGenAcc2DFine");
	TH2D * hEffAcc2DFine = (TH2D * ) fin->Get("hEffAcc2DFine");
	...
	for(int j = 0; j < NYBins; j++){
		...
		for(int i = 0; i < NPtBins+1; i++){
			...
			if(pt > ptmin && pt < ptmax){
				...
				NGen = hPtGen2DFine->GetBinContent(XBinNow,YBinNow);
				NPass = hPtGenAcc2DFine->GetBinContent(XBinNow,YBinNow);
				Acc = hEffAcc2DFine->GetBinContent(XBinNow,YBinNow);

				NGen2 = EffBptByGen->GetBinContent(XBinNow,YBinNow);
				NRECO = EffBptByMC->GetBinContent(XBinNow,YBinNow);
				SelEff = hEffSelectionFine->GetBinContent(XBinNow,YBinNow);
				Eff = hEff2DInv2Shots->GetBinContent(XBinNow,YBinNow);


				//		if(NPass > 0)	cout << "y from " <<  YBin[j] << "   -   " << YBin[j+1]  << "   pt from " << PtBin[i] << "   -   " << PtBin[i+1] << "   NGen =  " << NGen << "   NPass =  " << NPass << "   Acc = " << Acc << endl;  
				//		if(NPass > 0)	cout << "y from " <<  YBin[j] << "   -   " << YBin[j+1]  << "   pt from " << PtBin[i] << "   -   " << PtBin[i+1] << "   NGen2 =  " << NGen2 << "   NRECO =  " << NRECO << "   SelEff = " << SelEff << "   Eff = " << Eff << endl;  
